...
@HiltViewModel
class NotEnrolledViewModel @Inject constructor(): ViewModel(){
        ...
        try {
            viewModelScope.launch {
                when(event){
                    ...
                    is NotEnrolledEvent.Search -> { onSearch() }
                    is NotEnrolledEvent.NavItemClick -> { onNavClick(event.navItem) }
                    is NotEnrolledEvent.OnChangeSliderFilterState -> { onChangeSliderFilter(event.type, event.value) }
                }
            }
