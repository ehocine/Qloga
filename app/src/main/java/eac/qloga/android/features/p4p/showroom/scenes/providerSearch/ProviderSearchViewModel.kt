...
@HiltViewModel
class ProviderSearchViewModel @Inject constructor(): ViewModel(){

    companion object {
        const val TAG = "ProviderSearchViewModel"
    }
         ...
         try {
            viewModelScope.launch {
              when(event){
                  ...
                  is ProviderSearchEvent.Search -> { onSearch() }
                  is ProviderSearchEvent.NavItemClick -> { onNavClick(event.navItem) }
                  is ProviderSearchEvent.OnChangeSliderFilterState -> { onChangeSliderFilter(event.type, event.value) }
              }
            }
