...
class P4pProviderRepository @Inject constructor(@QLOGAApiService private val apiService: P4pProviderApi) {
    ...
    suspend fun getPrvRequestById(prvId: Long, rqid: Long) =
        apiService.getPrvRequestById(prvId, rqid)

    suspend fun getCustomerReviews(prvId: Long, cstId: Long) =
        apiService.getCustomerReviews(prvId, cstId)
    ...
    ) = apiService.getOrders(prvId, page, psize, filter?.toJsonString(), selector?.toJsonString())

    suspend fun workWithOrder(
        prvId: Long,
        requestBody: Order,
        action: PrvActionEnum,
        note: String,
        adata: OrderReview
    ) = apiService.workWithOrder(prvId, requestBody, action, note, adata.toJsonString())
    ...
    suspend fun workWithOrder(
        ...
        action: PrvActionEnum,
        note: String
    ) = apiService.workWithOrder(prvId, requestBody, action, note, null)
    ...
    suspend fun getAssigns(
        ...
        page: Long,
        psize: Long,
        filter: AssignSearchFilter?,
        selector: AssignFieldsSelector?
    ) = apiService.getAssigns(
        prvId,
        page,
        psize,
        filter?.toJsonStringWithDate(),
        selector?.toJsonString()
    )
    ...
    suspend fun addServiceConditions(
        prvId: Long,
        linkParam: Long,
        bodyParam: List<ProviderServiceCondition>
    ) = apiService.addServiceConditions(prvId, linkParam, bodyParam)

    suspend fun updateServices(prvId: Long, bodyParam: List<ProviderService>) =
        apiService.updateServices(prvId, bodyParam)

    suspend fun deleteServiceConditions(prvId: Long, bodyParam: List<ProviderService>) =
        apiService.deleteServiceConditions(prvId, bodyParam)

    suspend fun getFavCustomers(prvId: Long) = apiService.getFavCustomers(prvId)
    suspend fun addFavCustomers(prvId: Long, idNewFavsCst: Long) =
        apiService.addFavCustomers(prvId, idNewFavsCst)

    suspend fun deleteFavCustomers(prvId: Long, cstIdForDel: Long) =
        apiService.deleteFavCustomers(prvId, cstIdForDel)

    suspend fun updateFavCustomers(prvId: Long, listCstIdForUpdate: List<Long>) =
        apiService.updateFavCustomers(prvId, listCstIdForUpdate)
