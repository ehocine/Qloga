...
class MediaRepository(private val apiService: MediaApi){
    //Media
    suspend fun getMedias(page:Long,psize:Long)= apiService.getMedias(page,psize)
    suspend fun upload(fileData: String, access: AccessLevel, aid: Long?)= apiService.upload(fileData, access, aid)
    ...
    suspend fun getImageDataUrl(mid: Long,size: MediaSize?)= apiService.getImageDataUrl(mid, size)
    ...
    suspend fun getAvatarDataUrl(type: AvatarType, id: Long, size: MediaSize?)= apiService.getAvatarDataUrl(type, id,size)
    suspend fun getAvatar(otype: AvatarType, id: Long)= apiService.getAvatar(otype,id)
    suspend fun getAvatarPrescaled(otype: AvatarType, id: Long, ms: MediaSize)= apiService.getAvatarPrescaled(otype, id, ms)
